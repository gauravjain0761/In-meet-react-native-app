...
import { getUserInfo, patchUserPoint, selectToken } from '~/store/userSlice';
...
export default function ConfirmChatModal(props: IConfirmModal) {
  const { isVisible, onClose, onConfirm, recipientId, onPurchase, isChatUnLockBefore } = props;
  ...
  const handleConfirm = async () => {
    ...
    try {
      if (!isChatUnLockBefore) {
        const res = await paymentApi.chatWithOthers({ token, recipientId });
        if (res.status === 200) {
          onConfirm();
        }
    ...
    <ReactNativeModal
      ...
      <View style={styles.container}>
        <View style={styles.cardContainer}>
          <SubTitleTwo style={styles.titleText}>要與此人聊天嗎?</SubTitleTwo>
